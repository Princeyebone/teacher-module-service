#!/usr/bin/env python3
"""
Test script to verify session generation trigger functionality
"""
...
from schedule_utils import trigger_session_generation_if_ready

async def test_session_trigger():
    """Test session generation trigger functionality"""
    print("=== Testing Session Generation Trigger ===")
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            print("\n--- Test 1: Neither timetable nor academic calendar exists ---")
            result = await trigger_session_generation_if_ready(str(teacher.id), db)
            ...
            print("✅ Session trigger tests completed successfully!")
            return True
...
if __name__ == "__main__":
    success = asyncio.run(test_session_trigger())
    if success:
        print("\n🎉 All session trigger tests passed!")
    else:
        print("\n💥 Some session trigger tests failed!")
        exit(1)