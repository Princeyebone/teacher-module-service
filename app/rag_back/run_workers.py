...
Examples:
    ...
    python run_workers.py start
    python run_workers.py info
    python run_workers.py test
"""
...
def start_both_workers():
    ...
    worker_1_script = current_dir / "worker_1.py"
    worker_2_script = current_dir / "worker_2.py"
...
def test_redis_connection():
    ...
    try:
        ...
        from app.rag_back.rag_back import rag_redis_settings
...
def show_worker_info():
    ...
    try:
        ...
        sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        from app.rag_back.rag_back import worker_config_1, worker_config_2, rag_redis_settings
        
        print("\n" + "="*60)
        print("[CONFIG] ARQ WORKER CONFIGURATION - TEXT CHUNKING")
        print("="*60)
        print(f"Redis Host: {rag_redis_settings.host}")
        print(f"Redis Port: {rag_redis_settings.port}")
        print(f"Redis Database: {rag_redis_settings.database}")
        ...
        async def enqueue_task():
            ...
            try:
                ...
                job2 = await redis.enqueue_job(
                    ...
                    gcs_file_name,
                    metadata,
                    _queue_name="rag_queue_2"
                )
                
                print(f"[SUCCESS] Test jobs queued:")
                print(f"   - Job 1: {job1.job_id} (queue: rag_queue_1)")
                print(f"   - Job 2: {job2.job_id} (queue: rag_queue_2)")
